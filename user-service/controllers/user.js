...
const profile = async (req, res) => {
  const user = await User.findOne({ where: { UserId: req.user.userId } }).catch(
    (err) => {
      console.log(err);
    }
  );
...
const login = async (req, res) => {
  ...
  const match = await argon2
    .verify(user.Password, reqBody.password)
    .catch((err) => {
      console.log(err);
    });
...
const register = async (req, res) => {
  ...
  const existUser = await User.findOne({ where: { Username: username } }).catch(
    (err) => {
      console.log(err);
    }
  );
  ...
  const existEmail = await User.findOne({ where: { Email: email } }).catch(
    (err) => {
      console.log(err);
    }
  );
...
const updatePassword = async (req, res) => {
  ...
  const hashedPassword = await argon2.hash(password, 10).catch((err) => {
    console.log(err);
  });
  await User.update(
    {
...
const manageDeleteProfile = async (req, res) => {
  await User.destroy({ where: { UserId: req.params.id } }).catch((err) => {
    console.log(err);
  });
...
const getBreederProfile = async (req, res) => {
  const breeder = await BreederDetail.findByPk(req.user.userId).catch((err) => {
    console.log(err);
  });
  if (!breeder) {
...
const getBreederProfileById = async (req, res) => {
  const breeder = await BreederDetail.findByPk(req.params.id).catch((err) => {
    console.log(err);
  });
  if (!breeder) {
    return res.status(404).json({ message: "Breeder Profile not found" });
